...
def get_state():
    ...
    response = {"hue": hue, "saturation": saturation, "value": value, "status": strip.status}
...
def update_state():
    ...
    state = {"hue": strip.hsv[0], "saturation": strip.hsv[1], "value": strip.hsv[2], "status": strip.status}
    state.update(payload)

    # Validate payload, No Pydantic because of old Python :C
    if state["hue"] > 360 or state["hue"] < 0:
        return jsonify({"error": "Hue must be a value between 0 and 360!"})

    if state["saturation"] > 1 or state["saturation"] < 0:
        return jsonify({"error": "Saturation must be a value between 0 and 1!"})

    if state["value"] > 1 or state["value"] < 0:
        return jsonify({"error": "Value must be a value between 0 and 1!"})

    if type(state["status"]) != type(True):
        return jsonify({"error": "Status must be a boolean value!"})
...
def show_rainbow():
    ...
    if strip.event_running:
        return jsonify({"error": "Animation is currently running!"})

    if duration == 0 or duration > 60:
        return jsonify({"error": "You don't want to do that!"})
